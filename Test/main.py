...
def draw_menu():
    ...
    return start_rect, load_game_rect, exit_rect


def handle_menu_click(pos, start_rect, load_game_rect, exit_rect):
...
while running:
    if game_state == MENU:
        start_rect, load_game_rect, exit_rect = draw_menu()

        for event in pg.event.get():
            if event.type == pg.QUIT:
                running = False
            elif event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
                handle_menu_click(event.pos, start_rect, load_game_rect, exit_rect)
